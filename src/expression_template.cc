...
在整个表达式之前 (调用赋值操作符之前) 不要尝试计算表达式。在求值之前，必须记录哪些操作应用于哪些对象。操作在编译时确定，因此可以对模板参数进行编码。
@ref:
...
auto operator+(BinaryExpression::DExpression<A> a, BinaryExpression::DExpression<B> b) {
    return BinaryExpression::DBinaryExpression<decltype(a), decltype(b), BinaryExpression::Add>(a, b);
}
...
void run() {
    ...
    // DExpression<DBinaryExpression<DExpression<Var>, DExpression<Constant>, Add>>
